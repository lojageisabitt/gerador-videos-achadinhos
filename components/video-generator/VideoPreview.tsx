...
type Props = { images: UploadedImage[]; colors: ColorConfig; speed: Speed; content: ProductContent; animation: AnimationStyle };

export function VideoPreview({ images, colors, speed, content, animation }: Props) {
  ...
  return <section className={styles.previewColumn}>
    ...
    <div ref={frameRef} className={`${styles.phoneFrame} ${styles[animation]}`} style={{ "--video-bg": colors.background, "--video-accent": colors.accent, "--video-text": colors.text, "--slide-duration": `${duration}s` } as React.CSSProperties}>
      <div className={styles.ambient}><span /><span /><span /></div>
      <div className={styles.cinematicEffect} aria-hidden="true">{Array.from({ length: 18 }, (_, index) => <span key={index} style={{ "--particle": index } as React.CSSProperties} />)}</div>
